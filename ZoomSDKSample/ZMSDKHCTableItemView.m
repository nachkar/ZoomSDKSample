...
/*- (void)removeButtonWithTag:(NSInteger)tag{
    ZMSDKButton* theButton = [self viewWithTag:tag];
    if(theButton){
        if([theButton isKindOfClass:[ZMSDKButton class]])
            [theButton cleanUp];
        if([theButton superview])
...
- (void)updatePanelistUI{
    ...
    if(nameField)
    {
        ...
        nameField.stringValue = [NSString stringWithFormat:@"%@ : %@ s",displayName,formattedTime];
