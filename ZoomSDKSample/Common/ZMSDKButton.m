...
{//for tooptip rect error on some 10.11 machine
    ...
    {
        NSString* tip = [[self.toolTip retain] autorelease];
        self.toolTip = nil;
        self.toolTip = tip;
    }
}
...
- (void)drawBackground
{
    ...
    NSImage* bgImage = [self getDisplayBgImage];
    if(bgImage)
    {
        [bgImage drawInRect:self.bounds
                     topCap:self.topBackgroundImageCap
                  bottomCap:self.bottomBackgroundImageCap
                    leftCap:self.leftBackgroundImageCap
                   rightCap:self.rightBackgroundImageCap];
        return;
    }
...
- (NSImage*)getDisplayBgImage
{
    ...
    if(self.isEnabled==NO)
        bgImage = self.disableBackgroundImage?:bgImage;
    else if(self.isSelected)
        bgImage = self.selectBackgroundImage?:bgImage;
    else if(buttonCell.isHighlighted)
        bgImage = self.pressBackgroundImage?:bgImage;
    else if(self.hovered)
        bgImage = self.hoverBackgroundImage?:bgImage;
    else
        bgImage = self.normalBackgroundImage?:bgImage;
