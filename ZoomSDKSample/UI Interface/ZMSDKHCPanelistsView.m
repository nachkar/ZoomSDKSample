...
- (id)initWithFrame:(NSRect)frameRect
{
    ...
    if(self)
    {
        ...
        _timerUserArray = [[NSMutableDictionary alloc] init];
        [self initUI];
        return self;
    }
    return nil;
...
- (NSTableRowView*)tableView:(NSTableView *)tableView rowViewForRow:(NSInteger)row
{
    if(tableView == _panelistTableView)
    {
        ...
        if(!rowView)
        {
            rowView = [[[ZMSDKHCTableItemView alloc] initWithFrame:NSMakeRect(tableView.frame.origin.x, tableView.frame.origin.y, NSWidth(tableView.bounds), Panelist_Row_Height)] autorelease];
        ...
        {
            //Noel Achkar
            [rowView setUserInfo:userID withTime:[[_timerUserArray objectForKey:[NSString stringWithFormat:@"%i",userID]] intValue]];
            [rowView setSelected:NO];
            [rowView updateUI];
        }
        return rowView;
    }
    return [[[ZMSDKHCTableItemView alloc] initWithFrame:NSZeroRect] autorelease];
}
...
-(void)timerUpdated:(unsigned int)userID
{
    ...
    if ([[_timerUserArray allKeys] containsObject:user]) {
        NSNumber *time = [_timerUserArray objectForKey:user];
        int time2 = [time intValue] + 1;
        [_timerUserArray setObject:[NSNumber numberWithInt:time2] forKey:user];
    } else {
        [_timerUserArray setObject:[NSNumber numberWithInt:1] forKey:user];
    }
    [_panelistTableView reloadData];
}
